...
const { generateAndSaveMatchDetails } = require('../services/claudeDataService');
...
router.post('/:id/analyze', async (req, res) => {
  try {
    ...
    // Fetch or generate match details (H2H, stats, injuries, odds)
    if (!getH2H(h2hKey) || !getInjuries(fixtureId)) {
      if (hasRapidApi) {
        await fetchMatchDetails(fixture).catch(console.error);
      } else {
        await generateAndSaveMatchDetails(fixture).catch(console.error);
      }
    }
    ...
    // Extract form from stored team stats (set by claudeDataService or fetch from API)
    let homeForm = [], awayForm = [];
    const homeStatsData = getTeamStats(homeTeamId);
    const awayStatsData = getTeamStats(awayTeamId);
    ...
    if (hasRapidApi) {
      ...
      // Use form data generated by Claude (stored in team stats)
      homeForm = homeStatsData?._form || [];
      awayForm = awayStatsData?._form || [];
    }
